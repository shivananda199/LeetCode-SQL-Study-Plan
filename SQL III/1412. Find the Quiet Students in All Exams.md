...
emm.student_id,
s.student_name
FROM exam_min_max emm
JOIN Student s ON emm.student_id = s.student_id
WHERE emm.student_id NOT IN (SELECT student_id FROM exam_min_max WHERE score=max_score OR score=min_score)
ORDER BY emm.student_id;
```